...
uint64
uvmdealloc(pagetable_t pagetable, pagetable_t kernel_pagetable, uint64 oldsz, uint64 newsz)
{
  if(newsz >= oldsz)
    return oldsz;
...
int
uvmcopy(pagetable_t old, pagetable_t new, pagetable_t _new, uint64 sz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
